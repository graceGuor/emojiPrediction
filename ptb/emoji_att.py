...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, input_,dict_emb):
    ...
    output, state, output_att = self._build_rnn_graph(inputs, config, is_training)

    softmax_w = tf.get_variable(
        "softmax_w", [size, vocab_size], dtype=data_type())
    softmax_b = tf.get_variable("softmax_b", [vocab_size], dtype=data_type())
    logits = tf.nn.xw_plus_b(output_att, softmax_w, softmax_b, name='logits')
  ...
  def _build_rnn_graph_lstm(self, inputs, config, is_training):
    ...
    try:
        outputs_his = self.outputs
    except:
        outputs_his = []
        output = state[conf.num_layers - 1][1]
    ...
    return output, state, outputs_att
